...
##LVM SETUP##
cryptsetup -v -y -c aes-xts-plain64 -s 512 -h sha512 -i 5000 --use-random luksFormat /dev/$lvm; #encryption is setup only for 1 disk, so if you need to encrypt both just do it after installing gentoo
...
echo 'USE="-wayland -systemd -gnome -aqua -cdinstall -cdr -css -dvd -dvdr -a52 -clamav -coreaudio -ios -ipod -iee1395 -telemetry -emacs -xemacs -emboss -3dfx -emboss -altivec -smartcard -cups -ibm cryptsetup crypt device-mapper xinerama lvm"' >> /mnt/gentoo/etc/portage/make.conf; #better left alone unless you know what to do 
