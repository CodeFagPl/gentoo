...
echo 'USE="-wayland -systemd -gnome -aqua -cdinstall -cdr -css -dvd -dvdr -a52 -cjk -clamav -coreaudio -ios -ipod -iee1395 -telemetry -emacs -xemacs -emboss -3dfx -emboss -altivec -smartcard -cups -ibm minimal bash-completion sound alsa symlink cryptsetup crypt device-mapper lvm savedconfig zstd"' >> /mnt/gentoo/etc/portage/make.conf;
